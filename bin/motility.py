...
import time
import math
from FAST import plotparams as plotparams
...
class Motility:
    ...
    def wire_frame_links(self,depth=5):
        ...
        for d in range(1,depth+1):
            for f1 in range(len(self.frame_links)):
                possible_links = list(filter(lambda link:link.forward_link == None,self.frame_links[f1]))
    ...
    def make_frame_links(self):
        ...
        self.frame1.filaments = list(filter(lambda filament:filament.fil_length > self.min_fil_length,self.frame1.filaments))
        self.frame2.filaments = list(filter(lambda filament:filament.fil_length > self.min_fil_length,self.frame2.filaments))
        ...
        for i in range(len(self.frame1.filaments)):
            ...
            frame2_filaments = list(filter(lambda filament:np.sqrt(np.sum((filament.cm - filament1.cm)**2)) < self.max_velocity and np.fabs(filament.fil_length - filament1.fil_length) < self.max_length_dif ,self.frame2.filaments))
    ...
    def plot_2D_path_data(self, num_points, extra_fname=None):
        ...
        #Prepare the processed images
        self.path_img      = np.nan*np.ones((self.width,self.height),dtype=bool)
    ...
    def plot_length_velocity(self,header='',extra_fname=None,max_vel=2400, max_length= 10000,nbins=30, min_points=2, min_path_length = 5, weighted=True, percent_tolerance=500, print_plot=True, minimal_plot=False, maxvel_color='b', plot_xlabels = True, plot_ylabels = True, square_plot = True, plot_length_f=False, fit_f = 'exp',dpi_plot=200):
        ...
        l_bin_counts, l_bin_locs = np.histogram(self.full_len_vel[valid_filtered,0],bins=l_bin_edges,density=False)
        ...
        v_bin_counts, v_bin_locs = np.histogram(self.full_len_vel[valid_filtered,1],bins=v_bin_edges,density=True)
        ...
        if print_plot:
            if minimal_plot:
                ...
                l_bin_counts, l_bin_locs, l_patches = axHistx1.hist(1E-3*self.full_len_vel[valid_filtered,0],bins=l_bin_edges,density=False,orientation='vertical',color='gray')
                max_prob_l     = np.max(l_bin_counts)

                #Plot histogram with all mobile filaments
                v_bin_counts, v_bin_locs, v_patches = axHisty2.hist(self.full_len_vel[valid_mobile,1],bins=v_bin_edges,density=True,orientation='horizontal',color='gray')

                #Get the y-limit for y-histogram-2
                max_prob_a = axHisty2.get_xlim()[1]

                #Plot histogram without stuck filaments
                v_bin_counts, v_bin_locs, v_patches = axHisty1.hist(self.full_len_vel[valid_filtered,1],bins=v_bin_edges,density=True,orientation='horizontal',color='gray')
...
class Island:
    ...
    def skeletonize_filaments(self):
        ...
        self.filaments = list(filter(lambda fil:len(fil.contour) > 0,self.filaments))
    ...
    def remove_crossing_filaments(self):
        ...
        '''
        self.filaments = list(filter(lambda filament:filament.num_tips == 2,self.filaments))
